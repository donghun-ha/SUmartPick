...
async def select():
    ...
    sql = "SELECT O.Order_ID, O.Product_seq, O.User_ID, P.name, P.price, O.Order_date, O.Address, O.refund_demands_time, O.refund_time, O.payment_method, O.Arrival_Time, O.Order_state, O.Product_ID FROM orders as O, products as P, users as U where P.Product_ID = O.Product_ID and O.User_ID = U.User_ID"
    curs.execute(sql)
    rows = curs.fetchall()
    conn.close()
    print(rows)
...
### 머신러닝 테스트
@router.get("/mlminus")
async def mlminus(order_id: int):
    return {
        'result' : datetime.now(),
    }
